...
@torch.no_grad()
def quantize_linear_weight(layer, n_bits=8):
...
@torch.no_grad()
def quantize_llama_layer(layer, weight_quant="per_channel", act_quant="per_token", quantize_bmm_input=False):
    """Quantize a single LLaMA layer using the same logic as quantize_llama_like."""
    ...
    for proj_name in ["gate_proj", "up_proj", "down_proj"]:
        ...
        if proj is not None and isinstance(proj, torch.nn.Linear):
            setattr(layer.mlp, proj_name, W8A8Linear.from_float(
                proj, weight_quant=weight_quant, act_quant=act_quant
            ))
    # Quantize Attention
    for proj_name in ["q_proj", "k_proj", "v_proj", "o_proj"]:
        proj = getattr(layer.self_attn, proj_name, None)
        if proj is not None and isinstance(proj, torch.nn.Linear):
            setattr(layer.self_attn, proj_name, W8A8Linear.from_float(
                proj, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input
            ))
    return layer
