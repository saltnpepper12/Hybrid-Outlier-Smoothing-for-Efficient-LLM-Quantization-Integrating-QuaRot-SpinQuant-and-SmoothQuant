...
@torch.no_grad()
def smooth_linear_weight(layer, act_scales, alpha=0.5):
    # act_scales: tensor of shape [in_features]
    device, dtype = layer.weight.device, layer.weight.dtype
    act_scales = act_scales.to(device=device, dtype=dtype)
    weight_scales = layer.weight.abs().max(dim=0, keepdim=True)[0]
...
@torch.no_grad()
def smooth_llama_layer(layer, scales, alpha=0.5, layer_name=None):
    """Apply SmoothQuant to a single LLaMA layer using the same logic as smooth_lm."""
    assert isinstance(layer, LlamaDecoderLayer)
    if layer_name is None:
        raise ValueError("layer_name must be provided for scale lookup")
    # Attention
